#!/usr/bin/env bash
# userdata.sh - Secured Initialisation Script for Azure

set -euo pipefail
exec > >(tee /var/log/user-data.log) 2>&1

echo "🛡️ Starting  Server Initialization on Azure..."
...
if [ "${MONITORING_ENABLED}" = "true" ]; then
    ...
    # Install Azure Monitor Agent for Azure monitoring
    curl -sL https://aka.ms/azcmagent-install | sudo bash
...
echo "📊 Application accessible via Load Balancer"
echo "🔍 Monitoring endpoint: :9100/metrics"
...
echo "✅ Azure Server is ready for production workload!"