...
resource "null_resource" "prod_guardrail" {
  count = local.is_production && var.ssh_allowed_cidr == "0.0.0.0/0" ? 1 : 0

  provisioner "local-exec" {
    command = "echo 'SSH cannot be open to the world in production' && exit 1"
  }
}
